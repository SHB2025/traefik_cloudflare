...
services:
  traefik:
    ...
    labels:
      ...
      ### NAPOMENA / PLEASE NOTE:
      ### sljedeće Lablese aktivirajte samo ukoliko utvrdite da vaše web aplikacije podržavaju definisane role!
      ### Activate the following Labels only if you determine that your web applications support defined roles!
      ### Labelsi se aktiviraju tako što uklonite "#" ispred "- traefik..."
      ### Labels are activated by removing the "#" before "- traefik..."
      # HSTS
      ...
      - traefik.http.middlewares.gzip.compress.excludedcontenttypes=image/*
      # Brotli Kompresija
      #- traefik.http.middlewares.brotli.compress=true
      #- traefik.http.middlewares.brotli.compress.level=5
      # Keširanje statičkog sadržaja
      #- traefik.http.middlewares.static-cache.headers.customResponseHeaders.Cache-Control=max-age=31536000,public,immutable
      #- traefik.http.middlewares.static-cache.headers.customResponseHeaders.Expires=$(date -d '+1 year' --utc +'%a, %d %b %Y %H:%M:%S GMT')
      # Preload kritičnih resursa
      #- traefik.http.middlewares.preload.headers.customResponseHeaders.Link=</static/critical.css>; rel=preload; as=style, </static/critical.js>; rel=preload; as=script
      # Content Security Policy
      #- traefik.http.middlewares.csp.headers.contentSecurityPolicy=default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'
      # Referrer Policy
      #- traefik.http.middlewares.referrer-policy.headers.referrerPolicy=no-referrer-when-downgrade
      # Permissions Policy
      #- traefik.http.middlewares.permissions-policy.headers.permissionsPolicy=geolocation=(), microphone=(), camera=()
      # Lazy Loading za slike
      #- traefik.http.middlewares.lazyload.headers.customResponseHeaders.Lazy-Load=true
    networks:
      - traefik
    ports:
      - 80:80
